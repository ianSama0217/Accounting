...
import com.example.accounting.dao.AccountDao;
...
public class AccountServiceImpl implements AccountService {
    ...
    public BasicRes<Account> getById(int accountId) {
        return accountsDao.findById(accountId)
                ...
                .orElseGet(() -> BasicRes.<Account>fail(ReturnMsg.ACCOUNT_NOT_FOUND));
    }
    ...
    public BasicRes<List<Account>> getByUserId(int userId) {
        ...
        List<Account> list = accountsDao.findByUserId(userId);

        if (list.isEmpty()) {
            return BasicRes.<List<Account>>fail(ReturnMsg.ACCOUNT_NOT_FOUND);
        }
